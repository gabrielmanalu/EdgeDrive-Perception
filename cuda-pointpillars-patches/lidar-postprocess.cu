/*
 * lidar-postprocess.cu — EdgeDrive Perception: nuScenes PostProcess + NMS
 * =========================================================================
 *
 * Patched from NVIDIA-AI-IOT/CUDA-PointPillars for nuScenes dataset.
 *
 * Changes from original KITTI version:
 *
 *   lidar-postprocess.hpp (companion header):
 *     num_classes   : 3  → 10  (nuScenes 10-class detection)
 *     num_anchors   : 6  → 8   (4 sizes × 2 rotations)
 *     num_box_values: 7  → 9   (DeltaXYZWLHRBBoxCoder code_size=9)
 *     score_thresh  : 0.1 → 0.05
 *     nms_thresh    : 0.01 → 0.2
 *     dir_offset    : 0.785 → -0.785
 *     anchors[]     : KITTI 3-class → nuScenes 4-size anchors
 *
 *   NMS safety cap (this file, line ~467):
 *     Added bounds check on bndbox_num_ before NMS launch.
 *     Without this, out-of-range bndbox_num_ causes cudaErrorInvalidConfiguration
 *     crash in nms_launch() when running alongside camera_node in ROS2.
 *     The check skips the frame silently if bndbox_num_ is invalid.
 *
 * Model output layout (NCHW from mmdetection3d ONNX):
 *   cls_scores : [1, 80, 200, 200]  → channels = num_anchors × num_classes
